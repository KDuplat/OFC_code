...
int  colreadfull(int **rio, double **rdo, char ***rso, char **rco, char to, int *nlo, char *namo, int ncolo, char *separo, char modo, char *caro, int linemaxo) {
  ...
  char *str, *buf;   FILE *f ;  
  /* *rio=NULL;  *rdo=NULL; **rso=NULL; *rco=NULL; */
  
  ncar=strlen(caro);  buf=(char*) calloc(linemaxo, sizeof(char));    

  if( fopener(&f,namo,"r")==-1){ fprintf(stderr,"#colreadfull: file <%s> not found \n", namo); return(-1);   }
  ...
  *nlo=nl;  
  ...
  if(nl==0){ /* fprintf(stderr,"#colreadfull: no valid line in <%s> ('%c',%s) \n", namo,modo,caro); */ return(-1);  }
  else{
    ...
    if( nf<ncolo){ fprintf(stderr,"#colreadfull: field number is above max (%d>%d) \n", ncolo,nf); return(-1); }
