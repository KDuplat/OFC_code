...
void lat_vec(ivs *vo, ivs vao, ivs vbo, ivs vno){
  ivs vai,vbi,v;
  vai.x1=rfl(vao.x1,vno.x1);
vai.x2=rfl(vao.x2,vno.x2);
vai.x3=rfl(vao.x3,vno.x3);  
  vbi.x1=rfl(vbo.x1,vno.x1);
vbi.x2=rfl(vbo.x2,vno.x2);
vbi.x3=rfl(vbo.x3,vno.x3);  v.x1=vbi.x1-vai.x1;
v.x2=vbi.x2-vai.x2;
v.x3=vbi.x3-vai.x3;  (*vo).x1=lat_bcvec(v.x1,vno.x1);
(*vo).x2=lat_bcvec(v.x2,vno.x2);
(*vo).x3=lat_bcvec(v.x3,vno.x3);
}
...
inline int latsc_nc(ivs vo, ivs vno){  
  ivs v;
  v.x1 = rfl(vo.x1,vno.x1);
v.x2 = rfl(vo.x2,vno.x2);
v.x3 = rfl(vo.x3,vno.x3);   return( v.x1 + vno.x1*(v.x2 + vno.x2*v.x3) ); 
...
inline int latfcc_nc(ivs vo, ivs vno){
  ...
  vi.x1 = rfl(vo.x1,2*vno.x1);
vi.x2 = rfl(vo.x2,2*vno.x2);
vi.x3 = rfl(vo.x3,2*vno.x3);   v.x1 = iflr(vi.x1,2,&(vr.x1));
v.x2 = iflr(vi.x2,2,&(vr.x2));
v.x3 = iflr(vi.x3,2,&(vr.x3));   ind=vr.x1 + 2*vr.x2 + 4*vr.x3 ; 
...
inline void latfcc_cn(ivs *vo, int no, ivs vno){
  ...
  (*vo).x1= 2*v.x1 + vdec.x1;
(*vo).x2= 2*v.x2 + vdec.x2;
(*vo).x3= 2*v.x3 + vdec.x3;
}
...
void  lattice_parameters(lats *lato){
  ...
  lato->unit=unit;   lato->dimu.x1=unit*dim.x1;
lato->dimu.x2=unit*dim.x2;
lato->dimu.x3=unit*dim.x3;
...
void  lattice_fill(lats *lato){
  int i,a,p, n,t,nnei; ivs v, pos,dim, *nei; latpts *pt;
   ...
   for(i=0; i<n; i++){ pt=&( (lato->pt)[i] );
     ...
     for(p=0; p<nnei; p++){   pos.x1 = v.x1 + nei[p].x1;
pos.x2 = v.x2 + nei[p].x2;
pos.x3 = v.x3 + nei[p].x3;  pt->nei[p]=lat_nc(t,pos,dim); }
   }
}
...
void  lattice(lats *lato,char* typo, ivs dimo){
  ...
  lattice_defaultvalues(lato); sprintf(lato->type,"%s",typo);  (*lato).dim.x1 = dimo.x1;
(*lato).dim.x2 = dimo.x2;
(*lato).dim.x3 = dimo.x3;
...
void lattice_vr(lats *lato, vs vdeco, vs vao){
  ...
  n   =   lato->n    ;   lato->dec.x1 =vdeco.x1;
lato->dec.x2 =vdeco.x2;
lato->dec.x3 =vdeco.x3;
  fac =1./lato->unit ;   lato->alat.x1=vao.x1  ;
lato->alat.x2=vao.x2  ;
lato->alat.x3=vao.x3  ;
  
  Bpt(i,n,pt,lato, /* does not work on the same line: macro + vector */  
      pt->vr.x1=vao.x1*(fac*pt->v.x1 - vdeco.x1);
pt->vr.x2=vao.x2*(fac*pt->v.x2 - vdeco.x2);
pt->vr.x3=vao.x3*(fac*pt->v.x3 - vdeco.x3); 
      ); 
}
...
void lattice_lmpdat(lats* lato, lmpdats *lmpdato){
  ...
  lmpdato->atomtypes=1;    lmpdato->xlo.x1= lato->alat.x1 *(-lato->dec.x1             );
lmpdato->xlo.x2= lato->alat.x2 *(-lato->dec.x2             );
lmpdato->xlo.x3= lato->alat.x3 *(-lato->dec.x3             );
  lmpdato->bondtypes=1;    lmpdato->xhi.x1= lato->alat.x1 *(-lato->dec.x1 + lato->dim.x1);
lmpdato->xhi.x2= lato->alat.x2 *(-lato->dec.x2 + lato->dim.x2);
lmpdato->xhi.x3= lato->alat.x3 *(-lato->dec.x3 + lato->dim.x3);
